...
<h3 style="margin-left:50px">${dto.qa_title}</h3>
	...
	<span style="font-size: x-large">${dto.id}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span style="opacity:0.5">${dto.qa_date }</span>
 	...
 	<hr class="my-4">
		<a href="/ask/updateForm/${dto.qa_no}" style="margin-left:50px">글 수정 </a>
...
<script>
	$(function(){
		$("a[id]").click(function(){
			...
			$.ajax({url:"/ask/delete", data:"qa_no="+qa_no, method:"delete"}
			).done(function(){
				location.href="/ask/ask";
			})
			return false;
		})
		...
		$(".dbtn").click(function(){
			...
			$.ajax({url:"/ask/deleteComm/"+qa_comm,
				method:"delete"
		}).done(function(){
				location.reload();		
			});
