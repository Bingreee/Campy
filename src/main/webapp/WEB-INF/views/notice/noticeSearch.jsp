...
<header class="d-flex flex-wrap justify-content-center py-3 mb-4 border-bottom">
	...
	<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
		...
		<div align="right">
			<a href="/campy/main">main</a> <a href="/notice/noticeWrite">새글 등록</a>
		</div>
		...
		<c:if test="${count != 0 }">
			<table class="table table-hover">
				...
				<c:forEach items="${nList}" var="notice">
					<tr>
						<td>${notice.not_no}</td>
						<td>${notice.id }</td>
						<td><a href="/notice/noticeContent/${notice.not_no}">${notice.not_title}</a></td>
			...
			<div id="page">
				<c:if test="${begin > pageNum }">
					<a
						href="/notice/noticeSearch?p=${begin-1}&noticesearch=${noticesearch}&noticesearchn=${noticesearchn}">[이전]</a>
				</c:if>
				<c:forEach begin="${begin }" end="${end}" var="i">
					<a
						href="/notice/noticeSearch?p=${i}&noticesearch=${noticesearch}&noticesearchn=${noticesearchn}">${i}</a>
				</c:forEach>
				<c:if test="${end < totalPages }">
					<a
						href="/notice/noticeSearch?p=${end+1}&noticesearch=${noticesearch}&noticesearchn=${noticesearchn}">[다음]</a>
				</c:if>
			</div>
	...
	검색 조건에 맞는 글이 없습니다.
		...
		<div id="noticeSearch" align="center">
			<form action="/notice/noticeSearch">
