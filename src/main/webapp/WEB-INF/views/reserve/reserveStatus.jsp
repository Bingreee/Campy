...
<header class="d-flex flex-wrap justify-content-center py-3 mb-4 border-bottom" style="position:sticky;top:0px; z-index: 10;background-color:white;">
      <a href="/campy/main" class="d-flex align-items-center  me-md-auto text-dark text-decoration-none">
        <svg class="bi me-2" width="40" height="32"><use xlink:href="#bootstrap"></use></svg>
        <img src="../../../CampPhoto/Camping/CampyLogo2.png" style="width:50px;" alt="campy"/>
        <span href="/campy/main" style="font-weight:bold;font-size:30px;">Campy</span> 
      </a>
...
<div class="row">
	...
	<c:forEach items="${reserveList}" var="rStatus">
		<nav style="display: block; height: 150px; width: 700px;float:center; margin-left:170px; margin-bottom:100px">
  			<nav class="list-group-item list-group-item-action " > 
    			...
    			<a href="/reserve//reserveDetail/${rStatus.reserve_no }" >
    			...
    			<p class="mb-1" style="text-align : center; margin-left:130px">
					...
					<c:if test="${today >= end_date2 }">
					<center>
					<form action="/review/reviewWrite/${rStatus.c_no }" id="goReview" method="get"> 
	...
	<center>
		<div id="page" >
				<c:if test="${begin > pageNum }">
					<a href="/reserve/reserveStatus?p=${begin-1 }">[이전]</a>
				</c:if>
				<c:forEach begin="${begin }" end="${end}" var="i">
					<a href="/reserve/reserveStatus?p=${i}">${i}</a>
				</c:forEach>
				<c:if test="${end < totalPages }">
					<a href="/reserve/reserveStatus?p=${end+1}">[다음]</a>
				</c:if>
		</div>
		</center>
	</main>
...
$(function(){
	...
	$(".deleteReserve").click(function(){
		...
		let reserve_no = $(this).attr("id");
		 $.ajax({url:"/reserve/deleteReserve", 
